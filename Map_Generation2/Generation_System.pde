...
class room
{
  ...
  boolean mainpath, bonus, boss = false;
  ...
  boolean checkadjacent(room otherroom)
  {
    for (int a=0; a<connections.size(); a++)
    {
      if (otherroom.connections.get(a)==id)
        return true;
    }
    return false;
  }
  ...
  void draw()
  {
    ...
    //if (mainpath)
    //  text("Path", x+(x*cell_size)+32, y+(y*cell_size)+16);
    if (boss)
      text("Boss", x+(x*cell_size)+32, y+(y*cell_size)+32);
    if (bonus)
      text("Bonus", x+(x*cell_size)+32, y+(y*cell_size)+32);
  }
...
void generate()
{
  ...
  for (int a=0; a<closed.size(); a++)
  {
    ...
    rooms.get(closest).connections.add(closed.get(a));
    rooms.get(closed.get(a)).connections.add(closest);
  ...
  while (current!=0)
  {
    ...
    {
      if (rooms.get(rooms.get(current).connections.get(a)).distance<smallest)
      {
        smallest=rooms.get(rooms.get(current).connections.get(a)).distance;
        connection=rooms.get(rooms.get(current).connections.get(a)).id;
      }
    }
    current=connection;
  ...
  //If the amount of bonus objects is less than 2, place a bonus object on any rooms not marked as the main path that only has one connection, mark said connections as main path
  for (int a=0; a<rooms.size(); a++)
  {
    if (rooms.get(a).connections.size()==1 && rooms.get(a).boss==false && rooms.get(a).distance!=0)
    {
      bonusrooms.add(new bonus(a));
  ...
  //Grid cleanup
  //todo remake, it causes some weird issues with paths that are completely square
  ...
  /*- iterate through all rooms until the biggest distance is, mark the said room as the boss room
   - step backwards into the distance, marking the rooms as the main path
   - if the amount of bonus objects is less than 2, place a bonus object on any rooms not marked as the main path that only has one connection, mark said connections as main path
   - remove any rooms that are not on the main path and are not marked as bonus
   ...
   - if the amount of bonus objects is less than 2, place a bonus object adjacent to a teleporter with a distance greater then 3*/

  //Restart generation if requirements are not met 
  if (rooms.size()<12)
  {
    isrunning=false;
    noLoop(); //calling no loop so the draw method dosent get called while reinitiating the grid
    delay(1000);
    setup();
  }
  isrunning=false;
