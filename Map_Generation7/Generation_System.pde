...
class room
{
  ...
  void generate()
  {
    ...
    //Middle mechanics area generation
    if (teleporter || boss || distance==0)
    {
      int safesize=1;
      if (boss)
        safesize=3;
      int xmiddle=(x+x2)/2;
      int ymiddle=(y+y2)/2;
      for (int a=xmiddle-safesize; a<=xmiddle+safesize; a++)
      {
        for (int b=ymiddle-safesize; b<=ymiddle+safesize; b++)
        {
          grids.get((b*75)+a).blocked=true;
          grids.get((b*75)+a).r=100;
          grids.get((b*75)+a).g=100;
          grids.get((b*75)+a).b=100;
          neutralgrids.set(neutralgrids.indexOf((b*75)+a), -1);
        }
      }
    }
    ...
    if (bonus==false)
    {
      for (int a=0; a<(int)random(2, 12); a++)
      {
        ...
        do
        {
          random = (int)random(0, neutralgrids.size()-1);
        }
        while (neutralgrids.get(random)==-1);
        grids.get(neutralgrids.get(random)).state=0;
        boolean breakout=false;
        for (int b=0; b<neutralgrids.size(); b++)
        {
          if (neutralgrids.get(b)==-1)
            continue;
        ...
        if (grids.get(neutralgrids.get(random)).bonus || grids.get(neutralgrids.get(random)).teleporter)
          continue;
