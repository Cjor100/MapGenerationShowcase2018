ArrayList pathfind(int start, int end, int statecheck)
{
  //Variables
  ...
  //Grid Setup
  for (int a=0; a<=41; a++)//hight
  {
    for (int s=0; s<=74; s++)//width
    {
  ...
  while (open.size()!=0)
  {
    ...
    if ((current-1)>=0 && (navgrids.get(current).x-1==navgrids.get(current-1).x))
      ...
      adjacent[1]=-1;
    if ((current+75)<navgrids.size() && (navgrids.get(current).y+1==navgrids.get(current+75).y))
      adjacent[2]=current+75; //up
    else
      adjacent[2]=-1;
    if ((current-75)>=0 && (navgrids.get(current).y-1==navgrids.get(current-75).y))
      adjacent[3]=current-75; //down
    else
      adjacent[3]=-1;
    ...
    for (int a=0; a<=3; a++)
    {
      if (adjacent[a]!=-1)
      {
        if (grids.get(adjacent[a]).state==statecheck || (closed.contains(adjacent[a])))
        {
          continue;
        }
